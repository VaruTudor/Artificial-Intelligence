...
def visualize_map(myController: controller, myRepository: repository):
    pygame.init()
    run = True
    while run:
        ...
        # screen.blit(controller.map_with_drone(repository.map.image()), (0, 0))
        screen.blit(myController.mapWithDrone(myRepository.map.image()), repository.drone)
